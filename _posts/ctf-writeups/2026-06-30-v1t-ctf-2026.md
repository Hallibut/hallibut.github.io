...
Tìm tới vị chí của khóa này mình có thể thấy 8 byte `80ac bfef b`

`194 db01` chính là **Giá trị FILETIME.**

### Lùng CRC32 payload

Phần gợi ý có nói **"...with its physical-offset-sorted CRC32 payload...."**, điều này nghĩa là flag đã bị hash bằng cả giá trị **CRC32 payload** nào đó. 
...
- Trường **Number of Values** `04 00 00 00`, Số lượng value bên trong khóa này là **4 Values**.
- Trường **Value List Offset** `a0 30 18 00`, Địa chỉ trỏ đến Value List chứa 4 Value (`0x1840a0`).
...
Mình sẽ không theo thứ tự trong vaule list mà sẽ sắp 4 offset tăng dần rồi ghép data inline theo physical order: `d0 3e 17 cb`.

**Target CRC32 Hash:** d03e17cb

### **Tìm dữ liệu ẩn nằm trong Slack Space**

Bây giờ, mục tiêu của mình là rà quét toàn bộ file `NTUSER.DAT` để tìm một **khóa đang được allocated** mà tên của nó khi đem băm CRC32 sẽ khớp với chuỗi `d03e17cb` này.
...
Sau khi tìm được vị trí, mình tìm đến node key này để kiểm tra
...
- **`0c00 0000`**: Khai báo dung lượng là **12 bytes**.
