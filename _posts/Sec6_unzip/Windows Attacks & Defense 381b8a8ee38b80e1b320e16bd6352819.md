...
Mình được cung cấp 2 máy, 1 là máy của người dùng Bob có vẻ đã nằm sẵn trong active directory và máy còn lại là Kali Linux, mình sẽ sử dụng để crack mật khẩu offline
...
Trên Kali tạo một file txt và paste đoạn hash vừa rồi vào. Chạy câu lệnh hashcat sau sử dụng từ điển rockyou,txt, một từ điển nổi tiếng chuyên phá các mật khẩu yếu. (bạn có thể tìm thấy nó ở /usr/share/wordlists/rockyou.txt.gz trong máy kali)
...
- **SYSVOL (System Volume)** is a special shared folder, được tự động tạo ra trên tất cả các **DC** trong hệ thống mạng AD của Windows Server.
- Thư mục này chứa hai thành phần đặc biệt quan trọng để quản trị hệ thống mạng:
    - **Group Policy Objects (GPO):** Chứa các file cấu hình chính sách, bao gồm các template, file registry (registry.pol), và các file cấu hình bảo mật được dùng để áp đặt quy định xuống các máy trạm và tài khoản người dùng.
    - **Logon/Startup Scripts:** Chứa các tập lệnh (batch, PowerShell...) chạy tự động khi máy tính khởi động, tắt máy, hoặc khi người dùng đăng nhập/đăng xuất.
- Vị trí và cơ chế
    - Được cài đặt ở đường dẫn **C:\Windows\SYSVOL**.
    - Được hệ thống chia sẻ ra mạng nội bộ với đường dẫn **\\<domain-name>\SYSVOL** để các máy client có thể truy cập và tải policy về.
    - **Replication:** Để đảm bảo mọi DC đều áp dụng chung một tập hợp chính sách giống nhau, Windows Server sử dụng dịch vụ **DFS-R (Distributed File System Replication)** để liên tục đồng bộ các thay đổi trong SYSVOL. *(Lưu ý: Ở các phiên bản Windows Server cũ từ 2003 trở về trước, cơ chế này được đảm nhiệm bởi dịch vụ FRS - File Replication Service)*.
- Điểm quan trong của hình thứ tấn công này:
    - 2008, they introduced **Group Policy Preferences (GPP),** một tập hợp các extensions của Group Policy
    - One of them include the ability to store and use credentials.
    - Microsoft mã hóa mật khẩu của tài khoản có quyền Administrator và lưu trong SYSVOL nhưng lại dùng chung **một chìa khóa giải mã duy nhất** cho tất cả các hệ thống trên thế giới.
    - Sai lầm chí mạng hơn nữa là họ đã **đăng công khai** chìa khóa này lên mạng, cho phép bất kỳ ai cũng có thể copy về để tự giải mã và chiếm toàn quyền hệ thống.
...
- Inspect **EventID 4663 (An attempt was made to access an object),** sử dụng Dummy File chứa cấu hình giả mạo và không gắn nó với bất kỳ chính sách thực tế nào. Máy tính bình thường sẽ không bao giờ đụng đến nó. Nếu hệ thống ghi nhận có một tài khoản đang cố tình mở và đọc file này, đó gần như chắc chắn là hành vi của hacker.
...
- Use **Honey pot,** follow these mindsets when setup:
    - Create a account which provide semi-privileged, tốt nhất là **Service Account**, sau đó lưu mật khẩu (wrong password) của tài khoản này vào file XML trong thư mục SYSVOL.
    - Các tài khoản dịch vụ thường rất hiếm khi bị đổi mật khẩu. Hayx thiết lập sao cho thời gian đổi mật khẩu cuối cùng của tài khoản này cũ hơn ngày tạo file XML.
    - Cài đặt một dummy task thỉnh thoảng dùng tài khoản này để đăng nhập.
...
File bị đọc là Groups.xml và Access Mask **0x80** (**ReadAttributes)** mean some read the attribute inside.
...
- Use **Honey pot,** follow these mindsets when setup:
    - Chỉ link GPO mồi nhử với các máy chủ **không quan trọng**.
    - Phải có hệ thống tự động giám sát liên tục.
    - Tự động gỡ unlink GPO mồi nhử và disable user responsible for modified it immediate nếu phát hiện nó bị sửa đổi.
...
- Reason why shares folder accidentally open for everyone
    ...
    - Đặt tên thư mục có đuôi $ để làm ẩn trên Windows, nhưng các phần mềm dò quét của hacker vẫn nhìn thấy rõ mồn một.
...
Di chuyển vào thư mục này bằng lệnh cd, mình đã thử tìm **các file có chứa NetBIOS name of the domain.**

```bash
findstr /s /i /m "eagle" *
```

có hai file **connect.ps1** và **connect2.ps1**. dùng lệnh cat để Dxem nội dung 2 file, mình đã tìm thấy mật khẩu cho người dùng **Administrator2**
...
Mình đã viết lại đoạn script để có thể hiểu nó rõ hơn

![image.png](image%2032.png)

Sau đó mình chạy file này để dò các Object.
...
Kết quả là không thể dùng để đăng nhập được
...
Có thể thấy có 1 người dùng thông thường (chính là Rocky) đã truy cập thẳng vào root object của Domain
...
- Inspect **Event ID 4769 (A Kerberos service ticket was requested)** Tìm kiếm hành vi bất thường khi một tài khoản xin cấp vé truy cập dịch vụ cụ thể (TGS) mà bỏ qua bước bắt buộc là phải lấy vé xác thực danh tính tổng thể (TGT) trước đó.
...
Để tạo một vé, mình cần hash của **krbtgt** và **SID** of the entire domain. Đầu tiên là lấy mã hash của tài khoản **krbtgt** sử dụng kỹ thuật DCSync**:**
...
Dump hash credential của người dùng krbtgt
...
Get-NetUser -TrustedToAuth #tìm các tài khoản người dùng được cấu hình Constrained Delegation with Protocol Transition
```
